<template>
            ...
            <div class="flex flex-col-reverse md:flex-row shadow m-4  p-2 lg:p-6">
                <div class="w-full md:w-1/2 flex flex-col justify-evenly">
                    ...
                    <nuxt-link to="/contact"
                        class="px-3 py-2 md:mx-6 lg:mx-0 flex mb-2 bg-emerald-500 lg:w-3/12 items-center justify-center text-center font-semibold rounded-full text-white">Book Now</nuxt-link>
                </div>
    ...
    <div v-for="day in package2Data?.days" :key="day.title">
        <h2 class="text-2xl font-semibold lg:text-4xl px-10 bg-[#ffffff]">{{ day.title }}</h2>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-4 gap-4 md:gap-6 px-4 md:px-10  py-10 bg-[#ffffff]">
            <NuxtLink v-for="(place, index) in day.places" :key="index"
                ...
                <h1 class=" text-xl md:text-2xl bg-opacity-60 p-2 absolute bottom-4 rounded-full left-4 text-center  text-black bg-white md:opacity-0 md:group-hover:opacity-100 transition-all duration-500 ease-in-out">{{ place.name }}</h1>
            </NuxtLink>
        </div>
    </div>
